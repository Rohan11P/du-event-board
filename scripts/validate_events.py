"""
Validates data/events.yaml against the required event schema.
Run:  python scripts/validate_events.py
Exit: 0 on success, 1 on failure (with friendly error messages)
"""
...
VALID_CATEGORIES = {
    ...
    "Python", "R", "Open Source", "Community", "Workshop",
    "Conference", "Meetup", "Webinar", "Other"
}
...
def validate_event(event, index):
    ...
    if "tags" in event:
        if not isinstance(event["tags"], list):
            errors.append(f"{prefix} → 'tags' must be a list, got {type(event['tags']).__name__!r}")
        else:
            for i, tag in enumerate(event["tags"]):
                if not isinstance(tag, str):
                    errors.append(f"{prefix} → 'tags[{i}]' must be a string, got {type(tag).__name__!r}")
...
def main():
    ...
    if not isinstance(events, list):
        print("❌ data/events.yaml must contain a list under an 'events:' key or at the top level.")
        sys.exit(1)
